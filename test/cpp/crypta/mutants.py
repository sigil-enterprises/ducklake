...
---------------------------
Every mutant, in either roster, is the same record: `{name, file, why, reddens}`
- plus, in MUTANTS, an optional `src` naming which source directory `file` lives
in, and in EXTENSION_MUTANTS an `old`/`new` substitution pair. `old` is matched
EXACTLY and required to occur EXACTLY once, and `reddens` names the test cases
that must go red. MUTANTS carries no `old`/`new` in this fork - see the note
above the list - so it answers `list`, `names`, `spec`, `count` and `cases` here
and refuses `apply` and `resolve`. `spec` and `count` answer for a
...
# NO SUBSTITUTION BODIES IN THIS FORK. Every entry below keeps its `name`,
# `file`, `why` and `reddens` - which is what the roster is READ for, here and in
# ducklake-bench - and carries no `old`/`new` pair, because that pair quoted the
# provider's own source verbatim and the provider is not part of this repository
# (`.github/crypta-pin`). Nothing in this tree can apply the standalone roster
# anyway: `apply` copies every directory in SOURCE_DIRS, and `src/crypta` is not
# one this fork has. `require_body` below turns that into a loud refusal rather
# than a KeyError deep in a runner loop. EXTENSION_MUTANTS keeps its bodies:
# those quote `src/storage/` and `src/functions/`, which are in this tree.
MUTANTS = [
    {
...
def require_body(mutant):
    """
    A mutant's `old`/`new` pair, or a refusal if it has none.

    An entry with no bodies cannot be substituted or resolved, and the one thing
    it must not do is behave as though it had been: a KeyError reads as a bug in
    the runner, and a skip reads as a mutant that resolved. Both are the vacuous
    green this file exists to prevent, so say what is missing and stop.
    """
    if "old" not in mutant or "new" not in mutant:
        raise SystemExit(
            "mutant %s carries no substitution body in this fork - it can be "
            "listed and named here, but only applied and resolved where the "
            "source it edits lives (`.github/crypta-pin`)." % mutant["name"]
        )
    return mutant["old"], mutant["new"]
...
def resolve_roster(repo_root, roster):
    ...
    for mutant in roster:
        ...
        old, _new = require_body(mutant)
        try:
            with open(target, "r") as handle:
                occurrences = handle.read().count(old)
        except IOError:
            problems.append(
