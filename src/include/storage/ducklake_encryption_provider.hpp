...
public:
  ...
  WrapKeys(const vector<DuckLakeFileIdentity> &identities,
  ...
  virtual string UnwrapKey(const DuckLakeFileIdentity &identity,
                           const string &base64_value) = 0;
  ...
  virtual vector<DuckLakeRewrapResult>
  RewrapKeys(const vector<DuckLakeFileIdentity> &identities,
  ...
  using Factory = std::function<unique_ptr<DuckLakeEncryptionProvider>(
      string socket, string lake_id, int64_t ttl)>;
