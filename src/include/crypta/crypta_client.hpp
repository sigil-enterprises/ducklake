...
//! Speaks CryptaWireManifest@v3 over a Unix domain socket.
//!
//! Every batch request carries a `count` so crypta can refuse a frame whose
//! item count does not match. The reply echoes the identity, and every read
//! site verifies it rather than trusting positional order — a defence that
//! is immune to frame-shape confusion by construction.
//!
