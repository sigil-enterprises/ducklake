...
public:
	...
	// Shared guard extracted so the Appender fast path (WriteNewDataFilesWithAppender) and the SQL-batch path
	// (WriteNewDataFilesSqlBatch, via WrappedEncryptionKeyLiteral above) enforce the empty-wrapped-value refusal from
	// ONE place.
	//
	// Returns false (nothing to write - the caller should write SQL NULL / Appender Value()) when wrapped_base64 is
	// empty and the file has no key. Throws InternalException, same as WrappedEncryptionKeyLiteral, when wrapped_base64
	// is empty but the file DOES have a key. Otherwise returns true with out_value set to the raw (unescaped) wrapped
	// value.
