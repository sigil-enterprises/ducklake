...
//     its own `encryption_key`, and a sweep that covered only
//     `ducklake_data_file` would leave every delete-file read broken at
//     retirement. Rows with `end_snapshot` set are still live for TIME TRAVEL
//     and still name the old KEK, so the walk is deliberately snapshot-BLIND:
//     no `end_snapshot IS NULL` filter anywhere. Both omissions are silent
//     until the day the old KEK goes away, which is the day nothing can be done
//     about them.
//
...
//         leaves a committed PREFIX, never a torn row: the smallest unit
//         anything can observe is one row's `encryption_key` going from one
//         intact blob to another intact blob.
...
//       * There is NO cursor to persist, because the sweep is CONVERGENT. The
//       KMS
...
//       =
//         <the blob we read>` - so a row another writer changed under us is
//         left alone rather than clobbered with a blob rewrapped from a stale
//         value.
//
// WHAT IT DOES NOT DO
// -------------------
...
// before. A sweep that minted a snapshot would show up in every consumer's
// change feed as a modification that changed no data.
//
//===----------------------------------------------------------------------===//
...
//! One of the two catalog tables the sweep walks. The pair is written down
//! HERE, once, and the loop below iterates it - so "did you remember the delete
//! files" is answered by a list rather than by a second copy of the body.
//!
...
DuckLakeRewrapKeysBind(ClientContext &context, TableFunctionBindInput &input,
                       vector<LogicalType> &return_types,
                       vector<string> &names) {
  auto &catalog =
      DuckLakeBaseMetadataFunction::GetCatalog(context, input.inputs[0]);
    ...
    if (entry.first == "dry_run") {
      ...
      if (requested <= 0) {
        throw InvalidInputException(
            "ducklake_rewrap_keys: batch_size must be positive, got %lld",
            static_cast<long long>(requested));
      }
      result->batch_size = static_cast<idx_t>(requested);
    } else {
      throw InternalException(
          "Unsupported named parameter for ducklake_rewrap_keys");
    }
  }

  // REFUSE A NON-ENVELOPED LAKE — it is a refusal rather than a no-op on
  // purpose.
  //
  // A lake attached without ENCRYPTION_SOCKET / ENCRYPTION_LAKE_ID holds
  // PLAINTEXT keys in `encryption_key`, exactly as upstream does. There is
  // nothing here for a sweep to move, and every action this function could take
  // on such a lake is wrong: rewriting the column would destroy working keys,
  // and returning zero rows would tell an operator running a rotation that the
  // sweep RAN and found nothing - which is the exact reading under which they
  // would then go and retire a KEK. Silence is the dangerous answer here, so it
  // says so instead.
  //
  ...
  if (!ducklake_catalog.EncryptionProvider()) {
    throw InvalidInputException(
        "ducklake_rewrap_keys: this lake was attached without "
        "ENCRYPTION_SOCKET / ENCRYPTION_LAKE_ID, so its "
        "encryption keys are plaintext and there is no KEK to rewrap them "
        "onto. Refusing rather "
        "than reporting an empty sweep: an empty sweep reads as 'nothing left "
        "to do', which is exactly the "
        "reading under which a KEK gets retired and every row in the lake is "
        "stranded");
  }
...
DuckLakeRewrapKeysInit(ClientContext &context, TableFunctionInitInput &input) {
...
static void RequireSuccess(QueryResult &result, const string &what) {
  if (result.HasError()) {
    throw IOException("ducklake_rewrap_keys: %s failed: %s", what,
                      result.GetError());
  }
}
...
//! progress at all.
static void SweepTable(Connection &connection, DuckLakeCatalog &catalog,
                       DuckLakeEncryptionProvider &provider,
                       const RewrapTarget &target,
                       const RewrapKeysBindData &bind_data,
                       vector<RewrapReportRow> &report) {
  ...
  while (true) {
    auto select = StringUtil::Format(
        "SELECT %s, table_id, path, encryption_key FROM %s.%s WHERE "
        ...
        static_cast<long long>(first_page ? std::numeric_limits<int64_t>::min()
                                          : last_id),
        target.id_column, static_cast<uint64_t>(bind_data.batch_size));
    first_page = false;
    auto page = connection.Query(select);
    RequireSuccess(
        *page, StringUtil::Format("reading a page of %s", target.table_name));
    ...
    for (auto &row : *page) {
      ...
      // retire the outgoing KEK, and this row would be the one that is
      // stranded. Naming it is the only useful answer.
      if (!DuckLakeEncryptionProvider::LooksWrapped(blob)) {
        throw IOException(
            "ducklake_rewrap_keys: %s file %s (table %lld) carries a plaintext "
            "encryption key, not a "
            "KMS-wrapped one. Refusing to report this sweep as complete: there "
            "is no KEK this row can "
            "be moved from, so retiring the outgoing KEK after a sweep that "
            "walked past it would strand "
      ...
      if (!DuckLakeEncryptionProvider::IsBase64(blob)) {
        throw IOException("ducklake_rewrap_keys: %s file %s (table %lld) "
                          "carries an encryption key that is "
                          "not base64. A wrapped key that cannot even be "
                          "decoded is either corruption or an "
                          "attempt to write something other than a key into "
                          "the catalog, and neither is "
                          "worth sending to the KMS",
                          target.is_delete_file ? "delete" : "data",
                          stored_path, static_cast<long long>(table_id));
      }
      ...
      identities.push_back(catalog.BuildEncryptionIdentity(
          TableIndex(static_cast<idx_t>(table_id)), stored_path,
    ...
    if (results.size() != identities.size()) {
      throw IOException(
          "ducklake_rewrap_keys: KMS answered %llu items for %llu files",
    ...
    for (idx_t i = 0; i < results.size(); i++) {
      ...
      if (results[i].rewrapped && !bind_data.dry_run) {
        ...
        auto update = StringUtil::Format(
            "UPDATE %s.%s SET encryption_key = %s WHERE %s = %lld AND "
            "encryption_key = %s",
            prefix, target.table_name,
            DuckLakeUtil::WrappedEncryptionKeyLiteral(results[i].wrapped, true),
        ...
        RequireSuccess(*written, StringUtil::Format("rewriting a key in %s",
                                                    target.table_name));
...
static void DuckLakeRewrapKeysExecute(ClientContext &context,
  ...
  if (!state.executed) {
    ...
    if (!provider) {
      ...
      throw InvalidInputException(
          "ducklake_rewrap_keys: this lake has no encryption provider");
    }
    ...
    for (auto &target : REWRAP_TARGETS) {
      SweepTable(connection, ducklake_catalog, *provider, target, data,
                 state.report);
    }
    state.executed = true;
  }
...
DuckLakeRewrapKeysFunction::DuckLakeRewrapKeysFunction()
    : TableFunction("ducklake_rewrap_keys", {LogicalType::VARCHAR},
                    DuckLakeRewrapKeysExecute, DuckLakeRewrapKeysBind,
