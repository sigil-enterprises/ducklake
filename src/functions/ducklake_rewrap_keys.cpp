...
static unique_ptr<FunctionData> DuckLakeRewrapKeysBind(ClientContext &context, TableFunctionBindInput &input,
	...
	if (!ducklake_catalog.EncryptionProvider()) {
		throw InvalidInputException("ducklake_rewrap_keys: this lake was attached without "
...
static void SweepTable(Connection &connection, DuckLakeCatalog &catalog, DuckLakeEncryptionProvider &provider,
	...
	while (true) {
		auto select =
		    StringUtil::Format("SELECT %s, table_id, path, encryption_key FROM %s.%s WHERE "
		                       ...
		                       static_cast<long long>(first_page ? std::numeric_limits<int64_t>::min() : last_id),
		...
		for (auto &row : *page) {
			...
			if (!DuckLakeEncryptionProvider::LooksWrapped(blob)) {
				throw IOException("ducklake_rewrap_keys: %s file %s (table %lld) carries a plaintext "
		...
		for (idx_t i = 0; i < results.size(); i++) {
			...
			if (results[i].rewrapped && !bind_data.dry_run) {
				...
				auto update = StringUtil::Format(
				    ...
				    "encryption_key = %s",
				    prefix, target.table_name, DuckLakeUtil::WrappedEncryptionKeyLiteral(results[i].wrapped, true),
				    target.id_column, static_cast<long long>(file_ids[i]), DuckLakeUtil::SQLLiteralToString(blobs[i]));
