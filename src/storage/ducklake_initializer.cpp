...
void DuckLakeInitializer::LoadExistingDuckLake(DuckLakeTransaction &transaction) {
	...
	// Backfill-on-migration for lakes created before 'encryption_envelope' existed. See
	// BackfillEncryptionEnvelopeFlag's header comment for the full rationale.
	...
	// Backfill-on-migration, see above.
	//
	// Backfills only when THIS attach gives POSITIVE evidence either way - never a guess: -
	// catalog.EncryptionProvider() != nullptr: encryption_socket was supplied and resolved this attach, so the lake is
	// definitely enveloped -> backfill 'true'. - catalog.Encryption() == UNENCRYPTED: the persisted 'encrypted' tag
	// itself says this lake has no per-file keys at all, and envelope requires ENCRYPTED (necessary, not sufficient) -
	// so it is definitely NOT enveloped, regardless of whether this attach supplied encryption_socket -> backfill
	// 'false'. This is exactly the case that resolves a genuinely plain, never-crypta lake on its first normal ATTACH
	// since this guard shipped. - Otherwise (ENCRYPTED but encryption_socket not supplied this attach): we cannot tell
	// a genuinely enveloped lake attached without its socket apart from a plain-ENCRYPTED non-crypta lake - do NOT
	// backfill, and leave DuckLakeServerSideCommit::IsEnvelopedLake's fail-closed default (absent key => enveloped) in
	// force until a future attach does resupply encryption_socket. Guessing 'false' here would permanently and silently
	// un-refuse a genuinely enveloped lake - a strictly worse regression than the one this backfill exists to close
	// (ducklake#96, PR #95).
	if (!has_envelope_key) {
