...
void DuckLakeCatalog::RefuseWrappedKeyWithoutProvider(const string &file_path, const string &stored_key) const {
	...
	throw IOException("file %s carries a wrapped encryption key, but this lake was attached without "
	                  "ENCRYPTION_SOCKET / ENCRYPTION_LAKE_ID. Re-attach with the envelope options",
	                  file_path);
}
...
void DuckLakeCatalog::RefuseUnusableEncryptionKey(const string &file_path, const string &decoded_key) const {
	...
	throw InvalidInputException(
	    "file %s carries an encryption key of %llu bytes, which is not a length AES accepts (16, 24 or 32). The "
...
void DuckLakeCatalog::RequireEncryptedTempSpill(ClientContext &context) {
	...
	if (!option) {
		throw InvalidInputException(
		    "this DuckDB build has no `temp_file_encryption` setting, so an enveloped DuckLake cannot be "
		    "attached: an out-of-core query would spill decrypted rows to temp_directory in the clear");
	}
	...
	try {
		...
		throw InvalidInputException(
		    "this DuckLake carries a KMS envelope, so DuckDB's `temp_file_encryption` must be on before it is "
		    "attached - otherwise an out-of-core query spills decrypted rows to temp_directory in the clear. Turning "
		    "it on was refused: %s. This process has already written unencrypted temporary files, and DuckDB will "
		    "not encrypt a temp directory that is half plaintext. Attach the enveloped lake before running work that "
...
void DuckLakeCatalog::RefuseUnencryptedTempSpill(const string &what) const {
	...
	throw IOException(
	    "refusing %s on an enveloped DuckLake while `temp_file_encryption` is off - DuckDB would write any "
	    "buffer it cannot hold in memory to temp_directory in the clear, which puts decrypted rows on local disk "
	    "outside the envelope entirely. It was turned on for this process at ATTACH, so something has since turned "
...
string DuckLakeCatalog::ResolveStoredEncryptionKey(TableIndex table_id, const string &stored_path,
	...
	if (encryption_provider) {
		...
		decoded_key = encryption_provider->UnwrapKey(
		    BuildEncryptionIdentity(table_id, stored_path, is_delete_file), stored_key);
	} else {
