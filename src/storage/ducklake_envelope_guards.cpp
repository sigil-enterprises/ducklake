...
DuckLakeFileIdentity DuckLakeCatalog::BuildEncryptionIdentity(
    TableIndex table_id, const string &stored_path, bool is_delete_file) const {
...
void DuckLakeCatalog::RefuseWrappedKeyWithoutProvider(
    const string &file_path, const string &stored_key) const {
...
void DuckLakeCatalog::RefuseMissingEncryptionKey(
...
void DuckLakeCatalog::RefuseUnusableEncryptionKey(
    const string &file_path, const string &decoded_key) const {
  ...
  if (decoded_key.size() == 16 || decoded_key.size() == 24 ||
      decoded_key.size() == 32) {
    return;
  }
  throw InvalidInputException(
...
void DuckLakeCatalog::RequireEncryptedTempSpill(ClientContext &context) {
  ...
  if (!option) {
    throw InvalidInputException(
        "this DuckDB build has no `temp_file_encryption` setting, so an "
  ...
  try {
    ...
    throw InvalidInputException(
        "this DuckLake carries a KMS envelope, so DuckDB's "
...
string DuckLakeCatalog::ResolveStoredEncryptionKey(
    TableIndex table_id, const string &stored_path, const string &resolved_path,
    bool is_delete_file, const Value &stored_key_value) const {
  ...
  if (encryption_provider) {
    ...
    decoded_key = encryption_provider->UnwrapKey(
        BuildEncryptionIdentity(table_id, stored_path, is_delete_file),
        stored_key);
  } else {
