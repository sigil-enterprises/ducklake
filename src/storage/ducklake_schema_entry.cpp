...
optional_ptr<CatalogEntry> DuckLakeSchemaEntry::CreateTableExtended(CatalogTransaction transaction,
	...
	if (duck_catalog.DataInliningRowLimit(transaction.GetContext(), schema_id, DuckLakeTableIndex()) > 0) {
		DuckLakeUtil::ValidateNoInlinedSystemColumns(base_info.columns);
	}
	//! get a local table-id
	auto table_id = DuckLakeTableIndex(duck_transaction.GetLocalCatalogId());
...
optional_ptr<CatalogEntry> DuckLakeSchemaEntry::CreateTable(CatalogTransaction transaction,
	...
	auto table_data_path =
	    DataPath() + duck_catalog.GeneratePathFromName(table_uuid, base_info.GetQualifiedName().Name().GetIdentifierName());
...
optional_ptr<CatalogEntry> DuckLakeSchemaEntry::CreateFunction(CatalogTransaction transaction,
	...
	if (!HandleCreateConflict(transaction, info.type, info.GetQualifiedName().Name().GetIdentifierName(),
	                          info.on_conflict)) {
		return nullptr;
	}
	auto &duck_transaction = transaction.transaction->Cast<DuckLakeTransaction>();
...
optional_ptr<CatalogEntry> DuckLakeSchemaEntry::CreateView(CatalogTransaction transaction, CreateViewInfo &info) {
	...
	auto &duck_transaction = transaction.transaction->Cast<DuckLakeTransaction>();
	// get a local view-id
	auto view_id = DuckLakeTableIndex(duck_transaction.GetLocalCatalogId());
...
void DuckLakeSchemaEntry::Alter(CatalogTransaction catalog_transaction, AlterInfo &info) {
	...
	case AlterType::ALTER_TABLE: {
		...
		auto alter_name = alter.GetQualifiedName().Name().GetIdentifierName();
		...
		if (table_entry->type != CatalogType::TABLE_ENTRY) {
			throw BinderException("Cannot use ALTER TABLE on entry %s - it is not a table", alter_name);
		}
		...
		if (alter.alter_table_type == AlterTableType::RENAME_TABLE) {
			...
			if (StringUtil::Lower(alter_name) != StringUtil::Lower(new_table->name.GetIdentifierName()) && existing_table) {
				throw BinderException("Cannot rename table %s to %s, since %s already exists.", alter_name,
				                      new_table->name.GetIdentifierName(), new_table->name.GetIdentifierName());
			}
		}
	...
	case AlterType::ALTER_VIEW: {
		...
		if (view_entry->type != CatalogType::VIEW_ENTRY) {
			throw BinderException("Cannot use ALTER VIEW on entry %s - it is not a view", alter_name);
		}
		...
		if (alter.alter_view_type == AlterViewType::RENAME_VIEW) {
			...
			if (StringUtil::Lower(alter_name) != StringUtil::Lower(new_view->name.GetIdentifierName()) && existing_view) {
				throw CatalogException(
				    "Could not rename view \"%s\" to \"%s\": another entry with this name already exists!", alter_name,
				    new_view->name.GetIdentifierName());
			}
		}
	...
	case AlterType::SET_COLUMN_COMMENT: {
		...
		auto table_entry = GetEntry(catalog_transaction, CatalogType::TABLE_ENTRY, alter.GetQualifiedName().Name());
		if (table_entry->type != CatalogType::TABLE_ENTRY) {
			throw BinderException("Cannot comment on columns for entry %s - it is not a table", alter_name);
		}
		auto &table = table_entry->Cast<DuckLakeTableEntry>();
		auto new_table = table.Alter(transaction, alter);
		transaction.AlterEntry(table, std::move(new_table));
...
void DuckLakeSchemaEntry::DropEntry(ClientContext &context, DropInfo &info) {
	...
	if (catalog_entry->type != info.type) {
		throw CatalogException("Existing object %s is of type %s, trying to drop type %s",
		                       catalog_entry->name.GetIdentifierName(), CatalogTypeToString(catalog_entry->type),
