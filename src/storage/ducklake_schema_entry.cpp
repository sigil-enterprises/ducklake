...
void DuckLakeSchemaEntry::Alter(CatalogTransaction catalog_transaction, AlterInfo &info) {
	...
	case AlterType::ALTER_TABLE: {
		...
		if (table_entry->type != CatalogType::TABLE_ENTRY) {
			throw BinderException("Cannot use ALTER TABLE on entry %s - it is not a table", alter.GetQualifiedName().Name());
		}
		...
		if (alter.alter_table_type == AlterTableType::RENAME_TABLE) {
			...
			if (StringUtil::Lower(alter.GetQualifiedName().Name().GetIdentifierName()) != StringUtil::Lower(new_table->name.GetIdentifierName()) && existing_table) {
				throw BinderException("Cannot rename table %s to %s, since %s already exists.", alter.GetQualifiedName().Name(),
	...
	case AlterType::ALTER_VIEW: {
		...
		if (view_entry->type != CatalogType::VIEW_ENTRY) {
			throw BinderException("Cannot use ALTER VIEW on entry %s - it is not a view", alter.GetQualifiedName().Name());
		}
		...
		if (alter.alter_view_type == AlterViewType::RENAME_VIEW) {
			...
			if (GetEntry(catalog_transaction, CatalogType::VIEW_ENTRY, new_view->name)) {
				throw CatalogException(
				    "Could not rename view \"%s\" to \"%s\": another entry with this name already exists!", alter.GetQualifiedName().Name(),
				    new_view->name);
			}
		}
	...
	case AlterType::SET_COLUMN_COMMENT: {
		...
		auto table_entry = GetEntry(catalog_transaction, CatalogType::TABLE_ENTRY, alter.GetQualifiedName().Name());
		if (table_entry->type != CatalogType::TABLE_ENTRY) {
			throw BinderException("Cannot comment on columns for entry %s - it is not a table", alter.GetQualifiedName().Name());
		}
		auto &table = table_entry->Cast<DuckLakeTableEntry>();
		auto new_table = table.Alter(transaction, alter);
		transaction.AlterEntry(table, std::move(new_table));
...
void DuckLakeSchemaEntry::DropEntry(ClientContext &context, DropInfo &info) {
	...
	if (!catalog_entry) {
		...
		throw InternalException("Failed to drop entry \"%s\" - could not find entry", info.GetQualifiedName().Name());
	}
