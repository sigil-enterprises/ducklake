...
static void HandleDuckLakeOption(DuckLakeOptions &options, const string &option,
                                 const Value &value) {
  ...
  if (lcase == "data_path") {
    ...
    options.config_options["data_inlining_row_limit"] =
        value.DefaultCastAs(LogicalType::UBIGINT).ToString();
  ...
  } else if (lcase == "crypta_cache_ttl_seconds" ||
             lcase == "encryption_cache_ttl_seconds") {
    options.encryption_cache_ttl_seconds =
        BigIntValue::Get(value.DefaultCastAs(LogicalType::BIGINT));
    ...
    options.at_clause = make_uniq<BoundAtClause>(
        "version", value.DefaultCastAs(LogicalType::BIGINT));
  } else if (lcase == "snapshot_time") {
    if (options.at_clause) {
      throw InvalidInputException("Cannot specify both VERSION and TIMESTAMP");
    }
    options.at_clause = make_uniq<BoundAtClause>(
        "timestamp", value.DefaultCastAs(LogicalType::TIMESTAMP_TZ));
    ...
    options.config_options["write_deletion_vectors"] =
        BooleanValue::Get(value.DefaultCastAs(LogicalType::BOOLEAN)) ? "true"
                                                                     : "false";
    ...
    options.busy_timeout =
        UBigIntValue::Get(value.DefaultCastAs(LogicalType::UBIGINT));
  } else if (lcase == "ducklake_version") {
    auto version = DuckLakeVersionFromString(value.ToString());
    if (version < DuckLakeVersion::V1_0) {
      throw InvalidInputException("ducklake_version must be >= '1.0', got '%s'",
                                  value.ToString());
    }
...
DuckLakeAttach(optional_ptr<StorageExtensionInfo> storage_info,
  ...
  if (info.path.empty()) {
    ...
    if (!secret) {
      throw InvalidInputException(
          "Default secret was not found - either specify a path to attach to "
    ...
    if (!secret) {
      throw InvalidInputException(
          "Secret \"%s\" was not found - if this was meant to be a path to a "
...
static unique_ptr<TransactionManager> DuckLakeCreateTransactionManager(
