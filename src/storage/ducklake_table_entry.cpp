...
unique_ptr<CatalogEntry> DuckLakeTableEntry::AlterTable(DuckLakeTransaction &transaction, SetPartitionedByInfo &info) {
	...
	//
	// Unlike the inlined-row and column-stats leaks, there is nothing safe to degrade partitioning TO:
	// `ducklake_file_partition_value.partition_value` is how a partitioned scan resolves which files satisfy a
	// predicate at all (see the `partition_value IN (...)` filter this metadata manager builds, and the per-file key
	// list it rebuilds on read). Suppressing the value the way RedactStatsOnEnvelopedLake suppresses min/max would not
	// degrade the plan - it would silently break correctness. So: refuse enabling partitioning on an enveloped lake
	// instead, loud, at the one statement that asks for it - ALTER TABLE ... SET PARTITIONED BY with a non-empty key
	// list. Clearing an existing partition (an empty key list) is exempt: that removes the leak rather than creating
	// it, and an operator undoing a mistake on an enveloped lake must not be told the fix itself is refused.
	//
	// This does not by itself cover a table that was partitioned BEFORE the lake's encryption envelope was configured -
	// persisted partition metadata never runs back through this ALTER statement. That case is caught at the write
	// chokepoint instead: DuckLakeTransaction::AppendFiles and ::AddCompaction refuse to write a NEW partition value on
	// an enveloped lake regardless of how the partition was defined, in src/storage/ducklake_transaction.cpp. The two
	// guards are deliberately not merged: this one is loud at the moment partitioning is REQUESTED, the other is loud
	// at the moment a value would actually be WRITTEN.
