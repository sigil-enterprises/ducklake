...
DuckLakeTableEntry::DuckLakeTableEntry(Catalog &catalog, SchemaCatalogEntry &schema, CreateTableInfo &info,
                                       DuckLakeTableIndex table_id, string table_uuid_p, string data_path_p,
...
static void ReplaceColumnRefName(ParsedExpression &expr, const string &old_name, const string &new_name) {
	if (expr.GetExpressionType() == ExpressionType::COLUMN_REF) {
		...
		if (!colref.IsQualified() && StringUtil::CIEquals(colref.GetColumnName().GetIdentifierName(), old_name)) {
...
vector<string> DuckLakeTableEntry::GetPartitionSQLExpressions() const {
	...
	for (auto &field : partition_data->fields) {
		...
		auto col_name = KeywordHelper::WriteOptionallyQuoted(col.GetName().GetIdentifierName());
...
DuckLakePartitionField GetPartitionField(DuckLakeTableEntry &table, ParsedExpression &expr) {
	...
	case ExpressionType::FUNCTION: {
		...
		if (name == "bucket") {
			...
			if (arguments[0].GetExpression().GetExpressionType() != ExpressionType::VALUE_CONSTANT) {
				throw InvalidInputException("Bucket count must be a constant integer, got %s", expr.ToString());
			}
			if (arguments[1].GetExpression().GetExpressionType() != ExpressionType::COLUMN_REF) {
				throw InvalidInputException("Expected bucket(bucket_count, column), but got %s", expr.ToString());
			}

			auto &bucket_expr = arguments[0].GetExpression().Cast<ConstantExpression>();
			if (!bucket_expr.GetValue().DefaultTryCastAs(LogicalType::BIGINT)) {
				throw InvalidInputException("Bucket count must be an integer");
			}
			auto bucket_count = bucket_expr.GetValue().GetValue<int64_t>();
			if (bucket_count <= 0) {
			...
			column_name = GetPartitionColumnName(arguments[1].GetExpression().Cast<ColumnRefExpression>());
			break;
		}
		...
		column_name = GetPartitionColumnName(arguments[0].GetExpression().Cast<ColumnRefExpression>());
		break;
	}
	default:
		throw NotImplementedException(
...
unique_ptr<CatalogEntry> DuckLakeTableEntry::AlterTable(DuckLakeTransaction &transaction, AddFieldInfo &info) {
	...
	auto column_path = IdentifiersToStrings(info.column_path);
	auto &parent_id = GetFieldId(column_path);
	...
	for (auto &child : StructType::GetChildTypes(parent_id.Type())) {
		if (child.first == info.new_field.Name()) {
			...
			throw CatalogException("Failed to add field - field \"%s\" already exists in column \"%s\"",
			                       info.new_field.Name().GetIdentifierName(), column_path.back());
		}
	}
	...
	for (idx_t col_idx = 0; col_idx < current_field_ids.size(); col_idx++) {
		...
		if (child_field_id && StringUtil::CIEquals(field_id->Name(), column_path[0])) {
			auto new_field_id = field_id->AddField(column_path, std::move(child_field_id));
...
unique_ptr<CatalogEntry> DuckLakeTableEntry::AlterTable(DuckLakeTransaction &transaction, RemoveFieldInfo &info) {
	...
	if (info.if_column_exists) {
		removed_id_ptr = TryGetFieldId(column_path);
	} else {
		removed_id_ptr = GetFieldId(column_path);
	}
	if (!removed_id_ptr) {
		return nullptr;
	}
	...
	for (idx_t col_idx = 0; col_idx < current_field_ids.size(); col_idx++) {
		auto &field_id = current_field_ids[col_idx];
		if (StringUtil::CIEquals(field_id->Name(), column_path[0])) {
			auto new_field_id = field_id->RemoveField(column_path);
			auto &col = table_info.columns.GetColumnMutable(PhysicalIndex(col_idx));
			col.SetType(new_field_id->Type());
			new_field_ids->Add(std::move(new_field_id));
		} else {
...
unique_ptr<CatalogEntry> DuckLakeTableEntry::AlterTable(DuckLakeTransaction &transaction, RenameFieldInfo &info) {
	...
	auto column_path = IdentifiersToStrings(info.column_path);
	auto &renamed_id = GetFieldId(column_path);
	...
	for (auto &child : StructType::GetChildTypes(parent_id.Type())) {
		if (child.first == info.new_name) {
			throw CatalogException(
			    ...
			    column_path.back(), info.new_name.GetIdentifierName(), StringUtil::Join(parent_path, "."));
		}
	}
	...
	for (idx_t col_idx = 0; col_idx < current_field_ids.size(); col_idx++) {
		auto &field_id = current_field_ids[col_idx];
		if (StringUtil::CIEquals(field_id->Name(), column_path[0])) {
			auto new_field_id = field_id->RenameField(column_path, info.new_name.GetIdentifierName());
			auto &col = table_info.columns.GetColumnMutable(PhysicalIndex(col_idx));
			col.SetType(new_field_id->Type());
			new_field_ids->Add(std::move(new_field_id));
		} else {
