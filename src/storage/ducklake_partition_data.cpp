...
bool DuckLakePartitionUtils::TryComputeBucketValue(const Value &value, idx_t bucket_count, int32_t &out_bucket) {
	if (value.IsNull() || bucket_count == 0) {
		return false;
	}
	int32_t hash;
	switch (value.type().InternalType()) {
	case PhysicalType::BOOL: {
		int64_t v = value.GetValue<bool>() ? 1 : 0;
		hash = DuckLakeMurmur3::HashValue<int64_t>(v);
		break;
	}
	case PhysicalType::INT8: {
		int64_t v = value.GetValue<int8_t>();
		hash = DuckLakeMurmur3::HashValue<int64_t>(v);
		break;
	}
	case PhysicalType::INT16: {
		int64_t v = value.GetValue<int16_t>();
		hash = DuckLakeMurmur3::HashValue<int64_t>(v);
		break;
	}
	case PhysicalType::INT32: {
		int64_t v = value.GetValue<int32_t>();
		hash = DuckLakeMurmur3::HashValue<int64_t>(v);
		break;
	}
	...
	case PhysicalType::FLOAT: {
		double dval = static_cast<double>(value.GetValue<float>());
		if (dval == 0.0) {
			dval = 0.0;
		}
		int64_t bits;
		memcpy(&bits, &dval, sizeof(int64_t));
		hash = DuckLakeMurmur3::HashValue<int64_t>(bits);
		break;
	}
	case PhysicalType::DOUBLE: {
		double dval = value.GetValue<double>();
		if (dval == 0.0) {
			dval = 0.0;
		}
		int64_t bits;
	...
	case PhysicalType::VARCHAR: {
		...
		hash = DuckLakeMurmur3::Hash(reinterpret_cast<const uint8_t *>(str.data()), str.size());
		break;
	}
	default:
		// Unsupported type - skip pruning to remain correct.
		return false;
	}
	int32_t mask = NumericLimits<int32_t>::Maximum();
	int64_t masked = static_cast<int64_t>(static_cast<uint32_t>(hash) & static_cast<uint32_t>(mask));
	out_bucket = static_cast<int32_t>(masked % static_cast<int64_t>(bucket_count));
	return true;
}
