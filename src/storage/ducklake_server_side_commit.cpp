...
//
// Ducklake_commit takes a metadata SCHEMA name, not a DuckLake catalog name - it is designed to run without ever
// ATTACHing a DuckLake in this session at all (that is the point: a multi-engine writer stages rows over a plain SQL
// connection to the metadata store and calls this to finalize). So there is no DuckLakeCatalog guaranteed to exist here
// the way there is for every ducklake_* table function that takes a catalog name
// (DuckLakeBaseMetadataFunction::GetCatalog). What we CAN do: if a DuckLake happens to be attached in the calling
// context whose metadata schema matches this commit's, its EncryptionProvider is authoritative for whether this lake is
// enveloped, and we use it. A commit with no matching attachment (the fully catalog-less case) has no envelope
// information available to check here at all - closing that residual gap needs either a persisted per-lake encryption
// flag or a required parameter on ducklake_commit itself, which is out of scope for this fix; recorded, not silently
// assumed safe.
optional_ptr<DuckLakeCatalog> DuckLakeServerSideCommit::ResolveEnvelopedCatalog() {
	...
	for (auto &attached : db_manager.GetDatabases(context)) {
		...
		// Match on the metadata DATABASE, not schema.
		//
		// Ducklake_commit's first argument is the name a caller ATTACHes the metadata catalog under - METADATA_CATALOG
		// '<name>' - which DuckLakeStorage::SetOption stores as options.metadata_database, read back via
		// DuckLakeCatalog::MetadataDatabaseName(). MetadataSchemaName() is a different, independent option
		// (METADATA_SCHEMA, default "main") that this test never sets. Comparing against MetadataSchemaName() here
		// meant the match NEVER succeeded for the exact case this guard exists for - ducklake_commit('ssappend_meta',
		// ...) against an ATTACH using METADATA_CATALOG 'ssappend_meta' - so the guard silently never fired and
		// execution fell through to DuckLake's own internal partition/data-file consistency checks instead of ever
		// refusing. Also accept MetadataSchemaName() for a lake ATTACHed without METADATA_CATALOG (metadata hidden in
		// the current database's own schema), where metadata_schema_name legitimately names that schema.
...
void DuckLakeServerSideCommit::ReadCommitHeader() {
	...
	string data_path;
	// Default an unset path separator to "/".
	//
	// A real ATTACH's DuckLakeCatalog::Separator() is never empty - it falls back to the filesystem's own separator -
	// but a server-side committer staging directly into ducklake_staged_commit can leave path_separator NULL (many
	// external writers have no reason to populate a field they never look at). An empty separator used to reach
	// DuckLakeMetadataManager::StorePath(path, separator), which only guards against separator == "/" and otherwise
	// calls StringUtil::Replace(path, separator, "/") - an empty search string, which DuckDB's StringUtil::Replace
	// refuses with an unrelated internal error ("Invalid argument to StringUtil::Replace - empty FROM") instead of ever
	// reaching a DuckLake-level error message. Bug found because it masked
	// crypta_server_side_commit_partition_refusal.test's expected refusal message with this internal error instead.
	string separator = "/";
...
void DuckLakeServerSideCommit::ReadStagedDataFiles() {
	...
	// The envelope forbids partition VALUES in the catalog.
	//
	// This is the multi-engine chokepoint DuckLakeTransaction::AppendFiles/ AddCompaction cannot see: a server-side
	// commit built from files staged directly into ducklake_staged_data_file(_partition) by a writer that never ran
	// through DuckLake's own C++ write path at all (the intended use of ducklake_commit - Spark/Trino stage files and
	// call this table function to finalize). Applying the same guard here, right before the files enter local_changes,
	// closes that gap without duplicating its judgement - see
	// DuckLakeTransaction::RefusePartitionValuesOnEnvelopedLake.
	{
...
void DuckLakeServerSideCommit::ReadStagedCompactions() {
	...
	for (auto &kv : shells) {
		...
		// The envelope forbids partition VALUES in the catalog. Twin of the ReadStagedDataFiles guard above: a
		// compacted file staged directly by a non-DuckLake writer never runs through DuckLakeTransaction::AddCompaction
		// either.
...
unique_ptr<MaterializedQueryResult> DuckLakeServerSideCommit::ScanStagedTable(DuckLakeStagedTableType kind) {
	...
	// Skip InitializeScan on a never-appended staged table.
	//
	// A staging temp table that has never had a row appended to it has zero row groups at all (not one empty one) -
	// RowGroupCollection::InitializeScan unconditionally D_ASSERTs its root segment is non-null, so calling it here on
	// an untouched table (e.g. ducklake_staged_data_file_column_stats when a staged file carries no column stats)
	// crashes a debug build outright instead of returning zero rows the way a normal SQL SELECT over the same empty
	// table would. Guard it the same way a real query plan does: skip the scan and hand back an empty result.
